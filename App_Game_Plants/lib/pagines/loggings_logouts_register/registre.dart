...
class _RegistreState extends State<Registre> {
  ...
  static const String cloudinaryUrl = "https://api.cloudinary.com/v1_1/$cloudName/image/upload";
        ...
        child: Center(
          child: Column(
            children: [
              ...
              _buildTextField(correuController, 'Correu', 'Introdueix el teu correu'),
              _buildTextField(contrasenyaController, 'Contrasenya', 'Introdueix la contrasenya'),
              _buildTextField(edadController, 'Edat', 'Introdueix la teva edat'),
              _buildTextField(nacionalitatController, 'Nacionalitat', 'Introdueix la teva nacionalitat'),
              _buildTextField(codiPostalController, 'Codi Postal', 'Introdueix el teu codi postal'),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('Selecciona la teva imatge de perfil:', style: TextStyle(color: Colors.white)),
                    ...
                    Row(
                      children: [
                        ...
                        _selectedImage != null
                            ...
                            : const Text('Cap imatge seleccionada', style: TextStyle(color: Colors.white)),
                      ],
                    ),
                  ],
                ),
  ...
  Future<String?> _uploadImageToCloudinary(File imageFile) async {
    try {
      int timestamp = DateTime.now().millisecondsSinceEpoch ~/ 1000; // Generar timestamp
      ...
      if (response.statusCode == 200) {
        ...
        print("❌ Error al subir la imagen: ${response.statusCode} - ${response.statusMessage}");
        return null;
      }
    } catch (e) {
  ...
  void _registerUser() {
    ...
    int newId = (provider.getUsuaris.isNotEmpty
            ? provider.getUsuaris.map((u) => u.id).reduce((a, b) => a > b ? a : b)
            : 0) +
        1;
